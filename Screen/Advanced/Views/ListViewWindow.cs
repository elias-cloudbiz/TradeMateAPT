...
using System.Text;
using Terminal.Gui;
using static TMPFT.Screen.Scenarios;
...
namespace TMPFT.Screen.Advanced.Views
{
	...
	class ListViewWindow : Scenarios
    {
			...
			public List<Type> _scenarios = Scenarios.GetDerivedClasses<Scenarios>().OrderBy(t => Scenarios.ScenarioMetadata.GetName(t)).ToList();
			...
			internal class ScenarioListDataSource : IListDataSource
			{
				...
				public List<Type> ScenariosList
				{
					get => scenarios;
					set
					{
				...
				public int Count => ScenariosList != null ? ScenariosList.Count : 0;

				public int Length => len;

				public ScenarioListDataSource(List<Type> itemList) => ScenariosList = itemList;

				public void Render(ListView container, ConsoleDriver driver, bool selected, int item, int col, int line, int width, int start = 0)
				{
					container.Move(col, line);
					// Equivalent to an interpolated string like $"{Scenarios[item].Name, -widtestname}"; if such a thing were possible
					var s = String.Format(String.Format("{{0,{0}}}", -_nameColumnWidth), Scenarios.ScenarioMetadata.GetName(ScenariosList[item]));
					RenderUstr(driver, $"{s}  {Scenarios.ScenarioMetadata.GetDescription(ScenariosList[item])}", col, line, width, start);
				}
				...
				int GetMaxLengthItem()
				{
					...
					{
						var s = String.Format(String.Format("{{0,{0}}}", -_nameColumnWidth), Scenarios.ScenarioMetadata.GetName(ScenariosList[i]));
						var sc = $"{s}  {Scenarios.ScenarioMetadata.GetDescription(ScenariosList[i])}";
						var l = sc.Length;
						if (l > maxLength)
						{
							maxLength = l;
